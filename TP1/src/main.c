...
int main(void)
{
    ...
    // Probé usando Unsigned long long int para ambos factoriales pero me caía un warning por la máscara %d; busqué, se supone que la máscara para ese tipo de datos es %llu, pero eclipse no me lo toma como máscara válida, asique me tengo que limitar al int común para lograr la consigna de "no warnings".
    ...
    do
    {
        ...
        switch(opcion)
        {
            ...
            printf("El factorial de %d es: %d" ,primerOperando, factorialPrimerOperando);
